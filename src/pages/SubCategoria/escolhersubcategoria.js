...
export default function EscolherSubCategoria(props){
        ...
        <>
            <StyleView>
                <View style={styles.titleContainer}>
                    <Text style={styles.titleText}>SUBCATEGORIAS CATALOGADOS</Text>
                </View>
                <FlatList   
                        ...
                        return (
                            <View style={{display: "flex", alignItems: "flex-end", justifyContent: "center", margin: 'auto'}}>
                                <View style={styles.touchIcons}>
                                    <TouchableOpacity onPress={() => dispacth(actions.DELETAR_SUBCATEGORIAS_REQUEST({id: item.id}))}>
                                        <IconEnt name="trash" size={30} color="#000" />
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => nav.navigate("ChamadosStack", {screen: "EditarSubCategoria", params: {subcategoria: item}})}>
                                        <IconAnt name="edit" size={30} color="#000" />
                                    </TouchableOpacity>
                                </View>
                                <TouchableOpacity
                                    style={styles.buttonStyle}
                                    onPress={() => nav.navigate("ChamadosStack", {screen: "SubCategoriaEscolhida", params: {subcategoria: item.id}})}
                                >
